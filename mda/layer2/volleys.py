...
def detect_volleys(
    ...
    ``annotated_df``: original df with added columns:
      ``gap_us``, ``new_volley``, ``volley_id``.
    ...
    vstats_all = pd.concat(stats_parts, ignore_index=True) if stats_parts else pd.DataFrame()
...
def volley_size_distribution(volley_stats: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns DataFrame with columns: exchange, n_volleys, p50, p95, p99, p99_9.
    """
    records = []
